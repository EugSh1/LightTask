...
describe("Test operations with categories", () => {
    ...
    it("Should return a list of categories", async () => {
        const response = await request(baseURL).get("/category").set("Cookie", tokenCookie).expect(200);
    ...
    it("Should get a category by name properly", async () => {
        ...
        await request(baseURL).post("/category").send({ name: uniqueName }).set("Cookie", tokenCookie);

        const response = await request(baseURL).get(`/category/${uniqueName}`).set("Cookie", tokenCookie).expect(200);
        expectTypeOf(response.body).toMatchTypeOf<Category>();
    });
    ...
    it("Should add a category properly", async () => {
        ...
        const addedCategory = response.body.find((category: Category) => category.name === uniqueName);
    ...
    it("Should respond with an error if you try to create a category with a name already taken", async () => {
        const uniqueName = Date.now().toString();
        await request(baseURL).post("/category").send({ name: uniqueName }).set("Cookie", tokenCookie).expect(201);
        await request(baseURL).post("/category").send({ name: uniqueName }).set("Cookie", tokenCookie).expect(400);
    });
    ...
    it("Should respond with an error if you send an invalid data when adding a new category", async () => {
        ...
        await request(baseURL).post("/category").send({ name: "" }).set("Cookie", tokenCookie).expect(400);
    ...
    it("Should update a category properly", async () => {
        ...
        const allCategoriesResponse = await request(baseURL).get("/category").set("Cookie", tokenCookie);
    ...
    it("Should respond with an error if you send an invalid data when updating a category", async () => {
        ...
        await request(baseURL).put("/category").send({ name: "" }).set("Cookie", tokenCookie).expect(400);
        await request(baseURL).put("/category").send({ name: "test" }).set("Cookie", tokenCookie).expect(400);
        await request(baseURL)
            .put("/category")
            .send({ name: "t".repeat(25) })
            .set("Cookie", tokenCookie)
            .expect(400);
        await request(baseURL).put("/category").send({ name: "test", id: "" }).set("Cookie", tokenCookie).expect(400);
        await request(baseURL).put("/category").send({ name: "", id: "test" }).set("Cookie", tokenCookie).expect(400);
        await request(baseURL).put("/category").send({ id: "test" }).set("Cookie", tokenCookie).expect(400);
        await request(baseURL).put("/category").send({ id: "" }).set("Cookie", tokenCookie).expect(400);
    });
    ...
    it("Should delete a category properly", async () => {
        ...
        await request(baseURL).delete(`/category/${newCategoryId}`).set("Cookie", tokenCookie).expect(200);

        const getCategoriesResponse = await request(baseURL).get("/category").set("Cookie", tokenCookie);

        const deletedCategory = getCategoriesResponse.body.find((category: Category) => category.id === newCategoryId);
        expect(deletedCategory).toBeUndefined();
    });

    it("Should respond with an error if you submit a non-existent task id when deleting a category", async () => {
        await request(baseURL).delete("/category/non-existent-id").set("Cookie", tokenCookie).expect(500);
    });
