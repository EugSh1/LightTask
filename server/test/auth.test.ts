...
describe("Test operations with categories", () => {
    ...
    it("Should respond with an error if you send an invalid data when adding user", async () => {
        ...
        await request(baseURL).post("/user").send({ name: "" }).set("Cookie", tokenCookie).expect(400);
        await request(baseURL).post("/user").send({ name: "test" }).set("Cookie", tokenCookie).expect(400);
        ...
        await request(baseURL)
            ...
            .set("Cookie", tokenCookie)
            .expect(400);
        await request(baseURL).post("/user").send({ password: "test" }).set("Cookie", tokenCookie).expect(400);
        await request(baseURL).post("/user").send({ password: "" }).set("Cookie", tokenCookie).expect(400);
    });
    ...
    it("Should log in properly", async () => {
        ...
        const response = await request(baseURL).post("/user/login").send({ name, password }).expect(200);
    ...
    it("Should respond with an error if you send an invalid data when logging in", async () => {
        ...
        await request(baseURL).post("/user/login").send({ name: "" }).set("Cookie", tokenCookie).expect(400);
        await request(baseURL).post("/user/login").send({ name: "test" }).set("Cookie", tokenCookie).expect(400);
        await request(baseURL)
            .post("/user/login")
            .send({ name: "test", password: "1".repeat(5) })
            .set("Cookie", tokenCookie)
        ...
        await request(baseURL)
            ...
            .set("Cookie", tokenCookie)
            .expect(400);
        await request(baseURL).post("/user/login").send({ password: "test" }).set("Cookie", tokenCookie).expect(400);
        await request(baseURL).post("/user/login").send({ password: "" }).set("Cookie", tokenCookie).expect(400);
    });
    ...
    it("Should log out properly", async () => {
        const response = await request(baseURL).post("/user/logout").set("Cookie", tokenCookie).expect(200);
